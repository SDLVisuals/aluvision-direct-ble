 ...
 * sorted, not an image or a hardware wiring diagram. RGBW is based on the
 * supplied assembly reference. The SPI enclosure/port positions are indicative.
...
(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;else root.LightningReceiverVisual=api;}(typeof globalThis!=='undefined'?globalThis:this,function(){
  ...
  function rgbw(result,options){
    ...
    // The supplied assembly has a broad return loop and long fan-out cables.
    // Their sockets point along the controller axis, not down the SPI port axis.
    cable(result,'controller-to-splitter',curve([-80,44,11],[-363,-89,2],[-435,165,2],[-159,151,11]),5.7,options.identifying,'#E0E2DC');
    box(result,'splitter-body',-163,140,3,25,23,16,'#D8DBD4',5);
    const ends=[[15,203],[21,178],[27,153],[33,128]];
    ends.forEach(([x,y],index)=>{
      cable(result,'splitter-branch-'+(index+1),curve([-138,158-index*5,11],[-83,170-index*10,7],[x-42,y,8],[x,y,8]),3.1,options.identifying,'#DDE0D7');
      cylinder(result,'rgbw-connector-'+(index+1),x,y,8,7.8,24,options.identifying,'x');
    });
    ...
    // RGBW is one logical line: identification always covers the whole fan-out.
    ...
    result.metadata={type:'RGBW',logicalOutputs:1,physicalConnectors:4,selectedPort:null,
      activePorts:[],identifyingPorts:[],identifying:Boolean(options.identifying),reference:'supplied-rgbw-assembly',
      physicalPositionsConfirmed:true,label:'RGBW-receiver · één geheel',
      description:'Lange voeding, receiver en verdeelkabel. De vier aansluitingen volgen samen één kleur of effect.'};
  }
