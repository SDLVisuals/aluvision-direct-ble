...
(function (root, factory) {
  ...
  function plan(shape, lineCount, speed, delayMs) {
    ...
    // Row delay is a separate, literal interval. Do not let a short interval
    // turn the slow end of the speed slider into a sub-second flash.
    var duration = delay > 0 ? Math.max(0.06,delay*(0.7+1.2*Math.pow(1-normalized,2)))
      : 0.24+4*Math.pow(1-normalized,3);
    // 0 is the slowest movement, not pause. A 24-second event at the bottom
    // gives a long continuous fade; normal/show speeds retain their timing.
    duration = Math.max(duration,24*Math.pow(1-normalized,6));
  ...
  function rate(family, variant, lineCount, speed, delayMs) {
    ...
    return !shape ? 0 : 1/plan(shape,lineCount,speed,delayMs).period;
  }
