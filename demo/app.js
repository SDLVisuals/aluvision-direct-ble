...
(function () {
  ...
  const pixelSetup=window.LightningPixelSetup.create({mode:previewContext?'preview':nativeContext&&typeof runtime?.services?.configureOutputs==='function'?'native':'native-unavailable',
    ...
    onSave:async({receiverId,outputs})=>{
      ...
      if(nativeContext){
        if(typeof runtime?.services?.configureOutputs!=='function')throw Error('Verbind met je hoofdreceiver om de instellingen te bewaren.');
        ...
        if(!previewContext)throw Error('Verbind met je hoofdreceiver om de instellingen te bewaren.');
  ...
  function animationControls(effect) {
    ...
    const specs = [
      ...
      ['lineDelayMs','Pauze tussen lichtlijnen',0,5000,s.lineDelayMs??160,' ms','Een langere pauze laat de volgende lichtlijn later beginnen.'],
      ['delayMs','Pauze tussen lichtlijnen',0,10000,s.delayMs??300,' ms','Een langere pauze laat de volgende lichtlijn later beginnen.'],
  ...
  function categoryLabel(key) { return Library.categories.find(category=>category.key===key)?.title||({catalogue:'Alle',presets:'Mijn animaties'})[key]||'Animaties'; }
  function libraryTab() { return route.library==='all'?'catalogue':route.library; }
  ...
  function renderLayout() {
    ...
    return `<div class="editor-grid">${controlContext('layout')}<section class="editor-controls"><div><div class="section-heading"><h2>Hoe staan je LED-lines?</h2></div>${type==='SPI'?'<p class="layout-demo-help">Zo kan licht door je opstelling bewegen. Je huidige kleur of animatie blijft behouden.</p>':''}<div class="layout-grid">${choices.map(([id,name,desc])=>`<button class="layout-choice ${type==='SPI'?'layout-choice-spi':''}" data-action="set-layout" data-id="${id}" aria-pressed="${zone().layout===id}">${type==='SPI'?spiLayoutPreview(id):`<span class="layout-art ${id}" aria-hidden="true"><i></i><i></i><i></i></span>`}<b>${name}</b><small>${desc}</small></button>`).join('')}</div></div><section class="layout-receiver-section"><h3>Volgorde en receivers</h3>${receiverOrderMarkup()}<p class="order-help">${type==='RGBW'?'RGBW verandert per receiver als één geheel.':'De preview gebruikt alle actieve pixels en de ingestelde aansluitzijde per uitgang.'}</p></section></section></div>`;
  }
  function layoutReceiverActions() {
    const z=zone(),list=receivers();
    return `<section class="layout-receiver-actions" aria-label="Receivers toevoegen aan ${esc(z.name)}"><div class="label-row"><h2>Receivers in deze zone</h2><small>${receiverCount(list.length)} · ${z.type}</small></div><button class="button full" data-action="layout-receiver-add" data-zone="${esc(z.id)}"><span aria-hidden="true">＋</span> Receiver toevoegen</button></section>`;
  }
  function receiverOrderMarkup() {
    const list=receivers();
    return `<p class="order-help">Laat een lijn knipperen om haar te herkennen. Sleep aan de stippen om de volgorde te wijzigen. Open een receiver voor zijn instellingen.</p><div class="receiver-list receiver-order combined-receiver-order" aria-label="Receivervolgorde">${list.map((r,i)=>receiverCard(r,i)).join('')}</div><p class="order-status" role="status" aria-live="polite"></p>`;
  }
  ...
  function pinProtectionCard() {
    ...
    const message=!nativeContext?'Beschikbaar in de iPhone-app.':!selected?'Geef je stand eerst een naam.':pinProtectionLoading?'Beveiliging controleren…':pinProtectionError||(pinProtectionReconnect?'Verbind opnieuw met je hoofdreceiver.':!available?'De verbindingsdienst is niet beschikbaar.':current?.scope==='new-installation'?'Gebruik een PIN wanneer je de hoofdreceiver toevoegt.':'Eén PIN voor wifi en netwerk verwijderen.');
  ...
  async function refreshPinProtection({afterReconnect=false}={}){
    ...
    }catch(_){pinProtectionError='Nog niet bevestigd. Verbind met je hoofdreceiver en probeer opnieuw.';}
  ...
  function renderDemoWifi(){
    ...
    const mainReceiver=model.receivers.find(r=>r.role==='main'&&r.lifecycle==='added');
  ...
  function render({top=false,preserveScroll=true}={}) {
    ...
    if(route.screen==='animations'&&zone()&&receivers().length&&!activeEffect()){
      route={...route,screen:'effects',family:null,library:'catalogue',effectsReturn:'controls'};top=true;
    }
    ...
    if(route.screen==='stand')main.querySelectorAll('.stand-summary>div').forEach((tile,index)=>{
      ...
      button.setAttribute('aria-label',index?'Receivers in deze stand bekijken':'Zones in deze stand bekijken');
    ...
    main.querySelectorAll('[data-receiver-detail]').forEach(card=>{
      ...
      card.querySelector('.receiver-manage-content').insertAdjacentHTML('beforeend',`<section class="receiver-service-section" aria-label="Receiver en software"><h3>${r.role==='main'?'Hoofdreceiver':'Software'}</h3><p>${r.role==='main'?'Deze receiver verbindt je app met de andere ledlines.':'Updates gaan draadloos via je hoofdreceiver.'}</p><button class="button secondary full" data-action="receiver-update" data-id="${esc(r.id)}">Softwareversie en updates</button></section>`);
  ...
  function showReceiverAssignment(receiverId,targetZoneId=undefined) {
    ...
    showEffectDialog('Zone wijzigen',`<p><b>${esc(r.name)}</b> · ${esc(r.type)} · nu ${r.zoneId?`in ${esc(M.getZone(model,r.zoneId).name)}`:'nog niet aan een zone toegewezen'}. De koppeling, lichtinstellingen en eventuele hoofdreceiverrol blijven bewaard.</p><div class="assignment-choices" aria-label="Zone kiezen">${stand().zones.map(z=>{
  ...
  document.addEventListener('click',async event=>{
    ...
    try {
      ...
      if(action==='receiver-port-enabled'){
        ...
        if(nativeContext){
          if(typeof runtime?.services?.configureOutputs!=='function')return toast('Verbind met je hoofdreceiver om de uitgangen te wijzigen.');
