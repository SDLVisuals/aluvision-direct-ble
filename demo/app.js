...
(function () {
  ...
  function selector() {
    ...
    if(count===1)return `<section class="selection single-scope" aria-label="Je bedient de enige ledline"><span class="scope-line-icon" aria-hidden="true">${icon('light')}</span><span class="scope-single-copy"><b>${esc(t('scopeLine',{number:1}))}</b><small>${typeOf(list[0])==='SPI'?'Pixel LED · SPI':'RGBW'}</small></span><span class="selection-summary">${esc(t('scopeCountOne'))}</span></section>`;
  ...
  function showStandOverview(kind){
    ...
    }).join(''):list.map(r=>`<button class="stand-overview-row receiver-zone-row" data-action="receiver-move" data-id="${esc(r.id)}" aria-label="${esc(r.name)} · ${esc(current.zones.find(z=>z.id===r.zoneId)?.name||'Niet in een zone')} · zone wijzigen">${icon('receiver')}<span><b>${esc(r.name)}</b><small>${r.type} · ${esc(current.zones.find(z=>z.id===r.zoneId)?.name||'Niet in een zone')}</small><small class="receiver-zone-row-hint">Tik om aan een andere zone toe te wijzen</small></span><span class="receiver-zone-change-icon" aria-hidden="true">${icon('zones')}</span></button>`).join('');
    showEffectDialog(zoneOverview?'Zones in je stand':'Receivers in je stand',`<section data-stand-overview="${kind}"><p>${zoneOverview?'Tik op een zone om die ledlines te bedienen.':'Tik op een receiver om die aan een andere zone toe te wijzen.'}</p><div class="stand-overview-list">${entries||`<p>${zoneOverview?'Je hebt nog geen zones.':'Je hebt nog geen receivers toegevoegd.'}</p>`}</div>${!zoneOverview&&unassigned.length?`<p>${unassigned.length} ${unassigned.length===1?'receiver heeft':'receivers hebben'} nog geen zone.</p><button class="button secondary full" data-action="overview-receivers" data-id="unassigned">Niet ingedeelde receivers bekijken</button>`:''}<button class="button full" data-action="${zoneOverview?'overview-zone-new':'overview-receivers'}">${zoneOverview?'＋ Zone toevoegen':'Receivers beheren'}</button></section>`);
  }
