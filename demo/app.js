...
(function () {
  ...
  const iconPaths = {
    ...
    together:'M3 6h18M3 10h18M3 16h18M3 20h18M12 10v6',
  ...
  function nameOfSelection() {
    if(continuousZone())return 'Samen · één doorlopende ledline';
    if(selection().kind==='all')return `Samen · ${receivers().length} ledline${receivers().length===1?'':'s'}`;
  ...
  function selector() {
    ...
    return `<section class="selection" aria-label="Ledlines kiezen"><header><h2>Je bedient</h2><span class="selection-summary" role="status">${esc(nameOfSelection())}</span></header><div class="receiver-chips"><button class="selection-together" data-action="select" data-id="all" aria-label="Alle ledlines samen bedienen" aria-pressed="${all}">${icon('together')}<span>Samen</span><small>alle ledlines</small></button>${continuousZone()?'':receivers().map((r,i)=>`<button data-action="select" data-id="${esc(r.id)}" aria-label="${esc(ledlineName(r,i))} bedienen" aria-pressed="${!all && selection().receiverId === r.id}">${icon('light')}<span>${esc(ledlineName(r,i))}</span></button>`).join('')}</div><p class="mixed-note" ${mixedSelection()?'':'hidden'}>De ledlines hebben verschillende instellingen. Je volgende wijziging geldt voor je selectie.</p></section>`;
  }
  ...
  function render({top=false,preserveScroll=false}={}) {
    ...
    if(route.screen==='settings')main.querySelector('[data-action="help"]')?.insertAdjacentHTML('afterend',`<button class="menu-card" data-action="preferences-reset"><span class="menu-icon">${icon('settings')}</span><div><b>Standaardinstellingen herstellen</b><small>Alleen taal en thema van deze app</small></div>${icon('chevron')}</button>`);
    ...
    main.querySelectorAll('[data-receiver-detail]').forEach(card=>{
      ...
      if(r.role==='main')card.querySelector('.receiver-details').insertAdjacentHTML('beforeend',`<section class="receiver-service-section" aria-label="Hoofdreceiver en software"><h3>Hoofdreceiver</h3><p>Deze receiver verbindt je app met de andere ledlines.</p><button class="button secondary full" data-action="receiver-update" data-id="${esc(r.id)}">Softwareversie en updates</button></section>`);
  ...
  function liveRequest(receiver,patch) {
    const state=receiver.state||{},on=state.on!==false&&state.power!==false;
    const rawBrightness=Number(state.bri??state.brightness??100);
    if(!Number.isFinite(rawBrightness)||rawBrightness<0||rawBrightness>100)return null;
    const brightness=on?Math.round(rawBrightness):0;
    const base={standId:receiver.standId,receiverId:receiver.id,brightness,transitionMs:120};
    if(receiver.type==='SPI'){
      if(Object.keys(patch).some(key=>!['bri','brightness','on','power','rgbwLast'].includes(key)))return null;
      return {...base,kind:'SPI_BRIGHTNESS',channels:[]};
    }
    if(receiver.type!=='RGBW'||state.engine!=='STATIC')return null;
    const colour=state.colors?.[0]||C.hex([state.r??0,state.g??0,state.b??0]);
    if(!/^#[0-9a-f]{6}$/i.test(colour))return null;
    const rgb=state.rgbEnabled?.[0]===false?[0,0,0]:[1,3,5].map(index=>parseInt(colour.slice(index,index+2),16));
    const white=state.whiteEnabled?.[0]===false?0:Number(state.whiteChannels?.[0]??state.w??0);
    if(!Number.isInteger(white)||white<0||white>255)return null;
    return {...base,kind:'RGBW_STATIC',channels:[...rgb,white]};
  }
  function syncLiveStatus() {
    document.querySelectorAll('[data-live-status]').forEach(node=>{
      const targets=node.dataset.liveStatus==='stand'?standReceivers():receivers().filter(receiver=>selection().kind==='all'||receiver.id===selection().receiverId);
      ...
      let kind='idle',message='LED-voorbeeld · nog niet door de receiver bevestigd.';
      ...
      else if(failed){kind='failed';message=applied?`Deels bevestigd (${applied}/${targets.length}). Controleer de verbinding.`:'Niet bevestigd · controleer de verbinding en probeer opnieuw.';}
      else if(states.includes('pending')){kind='pending';message=`Versturen… ${applied}/${targets.length} bevestigd.`;}
      ...
      else if(applied===targets.length){
        kind='applied';
        const spi=targets.filter(receiver=>receiver.type==='SPI').length;
        message=spi===targets.length?`SPI-helderheid door receiver bevestigd · ${applied}/${targets.length}. Andere SPI-instellingen zijn alleen voorbeeld.`:
          spi?`RGBW-kleur en SPI-helderheid bevestigd · ${applied}/${targets.length}. SPI-kleur is alleen voorbeeld.`:
            `Kleur door receiver bevestigd · ${applied}/${targets.length}.`;
      }
      node.dataset.state=kind;node.textContent=message;
    });
  }
  function apply(patch) {
    const ids=selected().map(receiver=>receiver.id);
    ...
    if(Object.keys(patch).some(key=>key!=='rgbwLast'))for(const id of ids){
      const receiver=model.receivers.find(item=>item.id===id);
      if(!receiver)continue;
      const request=liveRequest(receiver,patch);
  ...
  document.addEventListener('click',async event=>{
    ...
    try {
      ...
      if(action==='preferences-reset')return showEffectDialog('Appvoorkeuren herstellen?',`<section data-preferences-reset><p>Alleen de appvoorkeuren veranderen: <b>Nederlands</b> en het <b>lichte thema</b>.</p><p>Je ${pinRequired?'PIN, ':''}receivers, zones, scènes, Mijn kleuren en animatiepresets blijven bewaard. Dit is geen fabrieksreset van je receivers.</p><p class="dialog-error" role="alert" hidden></p><button class="button full" data-action="preferences-reset-confirm">Appvoorkeuren herstellen</button><button class="button secondary full" data-action="effect-dialog-close">Annuleren</button></section>`);
      ...
      if(action==='preferences-reset-confirm'){
        ...
        return toast('Appvoorkeuren hersteld. Je installatie en opgeslagen licht blijven bewaard.');
      }
      ...
      if(action==='scene-apply'){const scene=savedScenes.scenes.find(s=>s.id===id&&s.standId===stand().id);if(!scene)return;model=Scenes.apply(model,scene);for(const receiver of standReceivers()){liveController?.preview(receiver.id);liveStates.set(receiver.id,{kind:'preview'});}render();toast(nativeContext?'Scène als voorbeeld getoond; nog niet op de receivers toegepast.':'Scène geactiveerd in het voorbeeld.');return;}
