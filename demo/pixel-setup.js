...
(function(root,factory){'use strict';const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;else root.LightningPixelSetup=api;}(typeof globalThis!=='undefined'?globalThis:this,function(){
  ...
  function strip(output,stage,options){
    ...
    return `<div class="pixel-setup-visual pixel-connection-preview" data-pixel-visual="connection" data-side="${reversed?'right':'left'}" data-preview-pixels="${output.pixels}" role="img" aria-label="${endpointLabel(output)}. De groene aansluiting is altijd bij de kabel; start 1 kan bij de kabel of aan het vrije uiteinde liggen. Schematisch, geen fysieke lichttest."><div class="pixel-setup-reference" aria-hidden="true"><span>${reversed?'Einde':'Start 1'}</span><i>${reversed?'←':'→'}</i><span>${reversed?'Start 1':'Einde'}</span></div><div class="pixel-cable-line"><span class="pixel-cable-source" aria-hidden="true"><svg viewBox="0 0 38 24"><rect x="1" y="3" width="23" height="18" rx="5"/><path d="M8 7h8M24 12h13"/></svg></span><span class="pixel-setup-track"><span class="pixel-setup-strip">${Array.from({length:count},(_,i)=>`<i${i===0?' class="start"':''}></i>`).join('')}</span></span></div><div class="pixel-setup-endpoint-labels"><span><i></i>Kabel</span><span>Vrij uiteinde</span></div></div>`;
  }
  ...
  function renderSide(output,{onboarding=false}={}){
    const attr=onboarding?'data-onboarding-action':'data-pixel-action';
    return `<section class="pixel-setup-panel" data-pixel-panel="connection"><h2>Waar begint jouw lijn?</h2><p>De kabel blijft op dezelfde plek. Jij kiest startpunt 1.</p><small class="pixel-preview-notice">Voorbeeld · geen testlicht</small>${strip(output,'connection')}<div class="pixel-setup-sides" role="group" aria-label="Beginpunt van de opstelling"><button type="button" ${attr}="side" data-side="left" aria-pressed="${!output.reversed}"><span class="pixel-start-choice" aria-hidden="true"><b>1</b><i>→</i></span><span>Bij de kabel</span></button><button type="button" ${attr}="side" data-side="right" aria-pressed="${output.reversed}"><span class="pixel-start-choice" aria-hidden="true"><i>←</i><b>1</b></span><span>Aan het vrije uiteinde</span></button></div><details class="pixel-setup-more"><summary>Waarom dit kiezen?</summary><p>Gebruik voor je opstelling hetzelfde startpunt voor iedere lijn, ook als de kabel aan de andere kant zit. Groen duidt hier alleen de fysieke aansluiting aan; het is geen gemeten of aangestuurd testlicht. De animatierichting kies je later.</p></details></section>`;
  }
