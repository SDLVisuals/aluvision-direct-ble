...
(() => {
  ...
  function scopedChange(callback, selectionPolicy = 'auto') {
    ...
    const draft = currentDraft();
    const desk = ensureDesk(draft);
    if (desk) desk.activePaletteId = '';
    window.AluvisionV20UiFixes?.invalidateActiveScene?.();
    ...
    try {
      ...
      // Some RGBW callbacks redraw Studio while the change is in progress.
      // Clear the palette marker again on the freshly-normalised desk so the
      // UI never claims a preset palette still matches a manually changed look.
      const currentDesk = ensureDesk();
      if (currentDesk) currentDesk.activePaletteId = '';
      document.querySelectorAll('.alv-pro-palette-choice.selected, .alv-pro-palette-choice[aria-pressed="true"]').forEach((button) => {
  ...
  function paletteButtons(compact = false) {
    ...
    const paletteMarkup = allPalettes().map((palette) => {
      const selected = activePaletteId === palette.id;
      const choice = `<button class="alv-pro-palette-choice ${selected ? 'selected' : ''}" aria-pressed="${selected}" data-palette-id="${esc(palette.id)}" onclick="studioProApplyPalette('${esc(palette.id)}')" title="${esc(palette.name)}"><i style="--palette:${palettePreview(palette)}"></i><span>${esc(palette.name)}</span></button>`;
      if (!palette.custom) return choice;
      return `<span class="alv-pro-palette-entry">${choice}<button type="button" class="alv-pro-palette-menu" aria-label="${tr('Palet beheren', 'Manage palette', 'Gérer la palette', 'Palette verwalten')}: ${esc(palette.name)}" onclick="studioProPaletteMenu('${esc(palette.id)}')">•••</button></span>`;
    }).join('');
    return `<div class="alv-pro-palettes ${compact ? 'compact' : ''}">${paletteMarkup}<button class="create" data-studio-action="palette-create" onclick="studioProSavePalette()"><i>＋</i><span>${tr('Huidige look', 'Current look', 'Look actuel', 'Aktueller Look')}</span></button></div>`;
  }
  ...
  style.textContent = `
    ...
    .alv-pro-palette-entry{position:relative;display:grid;min-width:130px;min-height:47px}.alv-pro-palette-entry>.alv-pro-palette-choice{position:relative;display:grid;grid-template-columns:31px minmax(0,1fr);gap:7px;align-items:center;width:100%;min-height:47px;padding:6px 39px 6px 6px;border:1px solid var(--line);border-radius:11px;background:var(--panel);color:var(--ink);text-align:left}.alv-pro-palette-entry>.alv-pro-palette-choice>i{display:grid;place-items:center;width:31px;height:31px;border-radius:9px;background:var(--palette);box-shadow:inset 0 0 0 1px #fff9,0 0 9px color-mix(in srgb,var(--palette),transparent 55%);font-style:normal}.alv-pro-palette-entry>.alv-pro-palette-choice>span{overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:8px;font-weight:900}.alv-pro-palette-entry>.alv-pro-palette-choice.selected{border-color:var(--pro-red);box-shadow:inset 0 0 0 1px var(--pro-red)}.alv-pro-palette-entry>.alv-pro-palette-choice.selected:after{content:'✓';position:absolute;right:4px;top:4px;display:grid;place-items:center;width:15px;height:15px;border-radius:6px;background:var(--pro-red);color:#fff;font-size:8px;font-weight:950}.alv-pro-palette-menu{position:absolute;z-index:2;right:5px;bottom:6px;display:grid;place-items:center;width:29px;height:29px;padding:0;border:0;border-radius:8px;background:var(--soft);color:var(--ink);font-size:8px;font-weight:950}.alv-pro-palette-menu:focus-visible{outline:2px solid var(--pro-red);outline-offset:2px}.alv-pro-programmer .alv-pro-palette-entry>.alv-pro-palette-choice{border-color:var(--pro-line);background:#ffffff09;color:#fff}.alv-pro-programmer .alv-pro-palette-menu{background:#ffffff10;color:#fff}.alv-pro-palettes.compact>.alv-pro-palette-entry{min-width:43px;width:43px}.alv-pro-palettes.compact>.alv-pro-palette-entry>.alv-pro-palette-choice{grid-template-columns:31px;width:43px;padding:6px}.alv-pro-palettes.compact>.alv-pro-palette-entry>.alv-pro-palette-choice>span,.alv-pro-palettes.compact>.alv-pro-palette-entry>.alv-pro-palette-menu{display:none}body.dark .alv-pro-palette-entry>.alv-pro-palette-choice{background:#252725}
