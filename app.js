 ...
 * Hardware management uses only typed native services. Light state changes
 * remain local until the separately verified live-control transport is wired. */
(function () {
  'use strict';
  ...
  // Sample installations are confined to the local development preview. The
  // same published assets start with an empty installation, just like iPhone;
  // a public hostname or query parameter can never imply discovered hardware.
  const previewContext=!nativeContext&&['localhost','127.0.0.1','[::1]','::1'].includes(window.location.hostname);
  let model = previewContext ? window.LightningFixtures.create() : M.assertValid({schemaVersion:30,demo:false,stands:[],receivers:[],scenes:[],presets:[]});
  let nativeLoaded=!nativeContext,nativeLoadError=false,nativeLoading=false;
  let route = {screen:'stand', zoneId:'zone-rgbw', family:null, library:'all'};
  ...
  try { presetStore = S.createStore(window.localStorage); } catch (_) { presetStore = S.createStore(null); }
  try { colourStore = Colours.createStore(window.localStorage); sceneStore=Scenes.createStore(window.localStorage); }
  ...
  let preferenceStore;try{preferenceStore=Preferences.createStore(window.localStorage);}catch(_){preferenceStore=Preferences.createStore(null);}
  ...
  const pixelSetup=window.LightningPixelSetup.create({mode:previewContext?'preview':'native-unavailable',
    ...
    onSave:({receiverId,outputs})=>{
      if(!previewContext)throw Error('De receiverinstellingen kunnen in deze bouw nog niet naar je receiver worden gestuurd. Er is niets gewijzigd.');
      ...
      model=M.assertValid(next);expandedReceivers.add(receiverId);
      ...
      render({preserveScroll:true});toast('Pixels en aansluitzijde aangepast in het voorbeeld.');
    }});
  const onboarding=window.LightningOnboardingUI.create({getModel:()=>model,
    services:runtime?.native===true?runtime.services||{}:{},
  ...
  const iconPaths = {
    ...
    together:'M3 5h18M3 12h18M3 19h18M7 4v16M17 4v16',
  ...
  function icon(name) { return `<svg class="icon" viewBox="0 0 24 24" aria-hidden="true"><path d="${iconPaths[name] || iconPaths.light}"/></svg>`; }
  ...
  function nameOfSelection() { return continuousZone()?'Eén doorlopende LED-line':selection().kind === 'all' ? `Samen · ${receivers().length} receivers` : selected()[0]?.name || 'Geen receiver'; }
  ...
  function selector() {
    ...
    return `<section class="selection" aria-label="Receiverselectie"><header><h2>Je bedient</h2><span class="selection-summary" role="status">${esc(nameOfSelection())}</span></header><div class="receiver-chips"><button data-action="select" data-id="all" aria-pressed="${all}">${icon('together')}Samen</button>${continuousZone()?'':receivers().map((r,i)=>`<button data-action="select" data-id="${esc(r.id)}" aria-pressed="${!all && selection().receiverId === r.id}"><span class="number">${i+1}</span>${esc(r.name)}</button>`).join('')}</div><p class="mixed-note" ${mixedSelection()?'':'hidden'}>Verschillende instellingen. Een wijziging geldt voor de hele selectie.</p></section>`;
  }
  function controlContext(screen) {
    ...
    return `<section class="control-context">${contextTitle(title,atRoot ? `${receivers().length} receivers · in ${standLabel()}` : z.name,atRoot ? 'Alle zones' : `Bediening · ${z.name}`,atRoot ? 'stand' : 'controls')}${atRoot ? `<div class="section-tabs" role="tablist" aria-label="Zonepagina"><button role="tab" data-action="controls" aria-selected="${screen === 'controls'}">${icon('sun')}Bediening</button><button role="tab" data-action="layout" aria-selected="${screen === 'layout'}">${icon('zones')}Opstelling</button></div>` : ''}<div class="preview-wrap" ${atRoot ? 'style="margin-top:13px"' : ''}><div class="preview-top"><span>${screen==='animations'?'Live LED-voorbeeld':'LED-voorbeeld'}</span><span class="preview-summary">${summary}</span></div>${zonePreview(z,'',{selection:selection(),main:true,label:`LED-voorbeeld van ${z.name}`})}${screen==='animations'?`<div class="preview-live-controls"><span>Kleuren en instellingen direct zichtbaar</span><button data-action="preview-motion" aria-label="Voorbeeld ${previewPlaying?'pauzeren':'afspelen'}" aria-pressed="${previewPlaying}">${previewPlaying?'Ⅱ Pauze':'▶ Afspelen'}</button></div>`:''}</div>${screen !== 'layout' ? selector() : ''}</section>`;
  }
  ...
  function renderStand() {
    ...
    return `<div class="page"><header class="page-heading"><div><div class="eyebrow">JOUW STAND</div><h1>${esc(standLabel())}</h1><p>Kies een zone en bedien meteen het licht.</p></div><button class="icon-button circle" data-action="help" aria-label="Uitleg over stand en zones">${icon('info')}</button></header><div class="stand-summary"><div>${icon('zones')}<span><b>${s.zones.length}</b><small>Zones</small></span></div><div>${icon('receiver')}<span><b>${added.length}</b><small>Receivers</small></span></div></div><section><div class="section-heading"><h2>Zones in deze stand</h2><button class="text-button" data-action="zone-new">＋ Nieuwe zone</button></div><div class="zone-grid">${s.zones.map(z=>`<article class="zone-entry"><button class="zone-card" data-action="zone" data-id="${esc(z.id)}">${zonePreview(z,'',{label:`Voorbeeld van ${z.name}`})}<div class="zone-copy"><div><b>${esc(z.name)}</b><span>${zoneTypeLabel(z)}${z.type?` · ${M.zoneReceivers(model,z.id).length} receivers`:''}</span></div><span class="open-label">${M.zoneReceivers(model,z.id).length?'Bedienen':'Instellen'} ${icon('chevron')}</span></div></button>${zoneDeleteButton(z)}</article>`).join('')}</div></section><aside class="guide">${icon('info')}<p><b>Een stand is je hele installatie.</b><br>Een zone is een plek, zoals de demohoek of de balie.</p></aside></div>`;
  }
  function powerControl() {
    const states = selected().map(r => r.state.on !== false && r.state.power !== false);
    const value = states.every(Boolean) ? true : states.some(Boolean) ? 'mixed' : false;
    return `<div class="power-card"><div><strong>${icon('power')}Verlichting</strong></div><button class="switch" role="switch" aria-label="${value==='mixed'?'Deels aan; zet de hele selectie aan':'Verlichting aan of uit'}" aria-checked="${value===true}" data-mixed="${value==='mixed'}" data-action="power"><span>${value === 'mixed' ? 'Deels aan' : value ? 'Aan' : 'Uit'}</span><i aria-hidden="true"></i></button></div>`;
  }
  function renderControls() {
    return `<div class="editor-grid">${controlContext('controls')}<section class="editor-controls">${powerControl()}<div class="control-menu"><button class="menu-card" data-action="colour"><span class="menu-icon colour-icon" aria-hidden="true"></span><div><b>Vaste kleur</b><small>Kies één kleur voor je selectie</small></div>${icon('chevron')}</button><button class="menu-card" data-action="animations"><span class="menu-icon">${icon('animation')}</span><div><b>Animaties</b><small>Kies een beweging en pas die aan</small></div>${icon('chevron')}</button></div><aside class="guide">${icon('light')}<p>${zone().type === 'RGBW' ? 'Iedere receiver bedient zijn aangesloten RGBW-verlichting als één geheel.' : continuousZone() ? 'Doorlopend is één LED-line. Je bedient alle delen samen. Kies Onder elkaar of Verticaal bij Opstelling om losse lijnen te bedienen.' : 'Kies Samen of één receiver. De actieve lijn is gemarkeerd in het voorbeeld.'}</p></aside></section></div>`;
  }
  ...
  function renderColour() {
    return `<div class="editor-grid">${controlContext('colour')}<section class="editor-controls">${colourPickerMarkup()}<p class="footer-note">W voegt wit licht toe. Combineer RGB en W voor een zachtere kleur.</p></section></div>`;
  }
  ...
  function colourPickerMarkup(slot=null) {
    ...
    return `<section class="card colour-card shared-colour-picker" data-colour-picker="${background?'background':slot===null?'static':'animation'}" data-slot="${slot??''}"><div class="section-heading"><h2>${background?'Achtergrondkleur':'Kleur kiezen'}</h2><small>Bekijk direct het resultaat</small></div><div class="colour-tools"><div class="wheel-wrap"><canvas class="wheel" id="colour-wheel" tabindex="0" role="img" aria-label="Kleurenwiel. Gebruik ook de RGBW-schuiven of de pijltjestoetsen." width="260" height="260"></canvas><span class="wheel-cursor"></span></div><div class="colour-values"><div class="colour-swatch" aria-label="Gekozen kleur"></div><p class="setting-hint">Tik of sleep om je kleur te kiezen. Verfijn hieronder.</p>${slot===null?'':`<small>Je wijzigt ${background?'de achtergrond':'deze animatiekleur'}. De animatie blijft actief.</small>`}</div></div>${slot===null?slider('bri','Kleurhelderheid',0,100,s.bri??100,'%'):''}<div class="fine-controls"><h3>RGBW · fijn afstellen</h3>${['r','g','b','w'].map((channel,i)=>`<div class="channel-row" style="--channel:${['#c4473f','#258461','#3f69c7','#747670'][i]}"><label for="${prefix}-${channel}">${channel.toUpperCase()}</label><button data-action="channel-step" data-channel="${channel}" data-step="-1" aria-label="${channel.toUpperCase()} verminderen">−</button><input id="${prefix}-${channel}" type="range" min="0" max="255" value="${i===3?white:rgb[i]}" data-channel="${channel}"><button data-action="channel-step" data-channel="${channel}" data-step="1" aria-label="${channel.toUpperCase()} verhogen">+</button><output data-channel-value="${channel}">${i===3?white:rgb[i]}</output></div>`).join('')}</div>${myColoursMarkup()}</section>`;
  }
  ...
  function animationControls(effect) {
    ...
    const specs = [
      ['speed','Snelheid',0,100,s.speed??30,'%', ''],
      ...
      ['lineDelayMs','Tijd tussen receivers',0,5000,s.lineDelayMs??160,' ms',''],
      ['delayMs','Tijd tussen receivers',0,10000,s.delayMs??300,' ms','Een langere tijd laat de volgende receiver later beginnen.'],
    ...
    return `<button class="settings-toggle" data-action="settings-toggle" aria-expanded="${settingsOpen}" aria-controls="animation-settings">${icon('sliders')}<span>Beweging instellen</span>${icon(settingsOpen?'close':'chevron')}</button><section id="animation-settings" class="card settings-panel" ${settingsOpen?'':'hidden'}>${specs.filter(spec=>available.includes(spec[0])).map(spec=>`<div class="visual-setting">${slider(...spec)}<button class="setting-reset" data-action="setting-reset" data-id="${spec[0]}" aria-label="${spec[1]} terug naar standaard">↺ Standaard</button></div>`).join('')}${available.includes('direction') ? `<div><p class="setting-hint">Bewegingsrichting</p><div class="compact-direction" aria-label="Bewegingsrichting">${(effect.directions||['right','left']).map(value=>`<button data-action="direction" data-value="${value}" aria-pressed="${(s.direction||effect.state.direction)===value}">${esc(directionMap[value]||value)}</button>`).join('')}</div></div>`:''}</section>`;
  }
  function renderAnimations() {
    const s = selectedState(), effect = activeEffect();
    if(!effect)return renderEffects();
    const content = `<div class="current-effect"><span class="menu-icon">${icon('animation')}</span><div><small>Actieve animatie · ${esc(categoryLabel(effect.category))}</small><b>${esc(effect.name)}</b><small>${esc(effect.description)}</small></div><button class="button" data-action="effects">Animatiegalerij ${icon('chevron')}</button></div>${effect.category==='brand'&&effect.controls.includes('brandColor')?brandControl(s.brandColor||'#C94E46','active'):''}<section class="card palette-section"><h2>${effect.paletteEditable===false?'Kleurenreeks':'Animatiekleuren · tik om te wijzigen'}</h2><div class="palette" aria-label="Animatiekleuren">${paletteMarkup(s)}</div>${slider('bri','Kleurhelderheid',0,100,s.bri??100,'%')}${resetMarkup('bri','Kleurhelderheid')}</section>${backgroundControls(effect)}${animationControls(effect)}<button class="button secondary full" data-action="preset-save">＋ Opslaan als eigen preset</button>`;
  ...
  function categoryLabel(key) { return Library.categories.find(category=>category.key===key)?.title||({catalogue:'Alle',presets:'Eigen presets'})[key]||'Animaties'; }
  function libraryTab() { return route.library==='all'?(zone().type==='SPI'?'pixels':'whole'):route.library; }
  ...
  function effectPreview(effect) {
    ...
    // Tunnel cards retain physical receiver order and the chosen orientation.
    // Even in continuous SPI mode a tunnel must show the separate light lines.
  ...
  function tunnelGuide() {
    return `<details class="tunnel-guide"><summary><span class="tunnel-glyph" aria-hidden="true">◠</span><span><b>Hoe werkt een tunnel?</b><small>Licht beweegt van receiver naar receiver</small></span>${icon('info')}</summary><div class="tunnel-explanation"><svg viewBox="0 0 320 150" role="img" aria-label="Uitlegvoorbeeld: vier lichtbogen achter elkaar vormen een tunnel"><path d="M28 138V91a132 83 0 0 1 264 0v47M64 138V95a96 65 0 0 1 192 0v43M100 138V99a60 46 0 0 1 120 0v39M129 138v-34a31 25 0 0 1 62 0v34" fill="none" stroke="#303d35" stroke-width="12"/>${[0,1,2,3].map((i)=>`<path class="tunnel-arch arch-${i}" d="${['M28 138V91a132 83 0 0 1 264 0v47','M64 138V95a96 65 0 0 1 192 0v43','M100 138V99a60 46 0 0 1 120 0v39','M129 138v-34a31 25 0 0 1 62 0v34'][i]}" fill="none" stroke="#bbe5cb" stroke-width="3"/>`).join('')}</svg><p>Elke lichtboog is één receiver. Het licht gaat bijvoorbeeld van de eerste naar de laatste boog, en weer terug.</p><p>Deze uitleg gebruikt vier bogen. De effectvoorbeelden hieronder tonen jouw <b>${receivers().length} receivers</b>, in de volgorde van Opstelling.</p></div></details>`;
  }
  ...
  function renderPresets() {
    ...
    if(savedPresets.error)return `<div class="card"><h2>Presets konden niet worden gelezen</h2><p>${esc(savedPresets.error.message)} Je bestaande opslag is niet gewijzigd.</p></div>`;
    if(!savedPresets.presets.length)return `<section class="card empty"><h2>Bewaar jouw eigen licht</h2><p>Kies eerst een animatie, stel de kleuren en beweging in en tik op <b>Opslaan als eigen preset</b>.</p><button class="button" data-action="library" data-id="${zone().type==='SPI'?'pixels':'whole'}">Een animatie kiezen</button></section>`;
  ...
  function effectCards(effects) {
    ...
    return effects.map(effect=>`<button class="effect-card" data-action="effect" data-id="${esc(effect.id)}" aria-pressed="${activeEffect()?.id===effect.id}" ${effect.minimumReceivers>1&&!together?'disabled':''}>${effectPreview(effect)}<b>${esc(effect.name)}</b><small>${esc(categoryLabel(effect.category))} · ${esc(effect.description)}</small>${effect.minimumReceivers>1&&!together?'<small>Kies Samen met minstens twee receivers</small>':''}</button>`).join('');
  }
  ...
  function renderEffects() {
    ...
    const hasAnimation=Boolean(activeEffect());
    const title=active?.title||(tab==='tunnel'||tab==='brand'||tab==='presets'?categoryLabel(tab):'Animatie kiezen');
    const canTunnel=receivers().length>=2&&selection().kind==='all',tabs=['catalogue','whole',...(zone().type==='SPI'?['pixels']:[]),'tunnel','brand','presets'];
    const intro=tab==='whole'||tab==='pixels'?`<p class="library-intro">${esc(Library.categories.find(category=>category.key===tab).summary)}</p>`:tab==='brand'?`<p class="library-intro">Rustige verlichting voor je stand: wit, warme RGB + W-mixen en subtiele kleuraccenten.</p>${brandControl(brandColours.get(route.zoneId)||'#C94E46','library')}`:tab==='tunnel'?tunnelGuide():tab==='catalogue'?`<p class="library-intro">${items.length} animaties · bestaande effecten én nieuwe V30-effecten. Gegroepeerd per familie.</p>`:'';
    let results=tab==='presets'?renderPresets():tab==='tunnel'&&!canTunnel?`<section class="card empty">${icon('together')}<h2>${receivers().length<2?'Minimaal 2 receivers nodig':'Een tunnel loopt tussen receivers'}</h2><p>${receivers().length<2?'Voeg later een tweede receiver aan deze zone toe.':'Je bedient nu één receiver. Kies bewust Samen om een tunnel voor de hele zone te gebruiken.'}</p>${receivers().length>=2?'<button class="button" data-action="tunnel-together">Bedien samen</button>':''}</section>`:active?`<p class="library-intro">${esc(active.summary)}</p><div class="effect-grid">${effectCards(active.effects)}</div>`:`<div class="family-grid animation-family-grid">${families.map(group=>`<button class="family-card animation-family-card" data-action="family" data-id="${esc(group.key)}">${effectPreview(group.preview)}<span class="family-category">${esc(group.categoryTitle)}</span><b class="family-title">${esc(group.title)}</b><small class="family-summary">${esc(group.summary)}</small><span class="family-variants">${esc(group.variantLabel)} ${icon('chevron')}</span></button>`).join('')}</div>`;
    return `<div class="page">${contextTitle(title,active?`${active.categoryTitle} · ${active.totalLabel} · ${zone().name}`:`${zone().name} · ${nameOfSelection()}`,active?'Alle effectfamilies':hasAnimation?'Terug naar instellingen':'Terug naar bediening',active?'effects-root':hasAnimation?'animations':'controls')}<div class="filter-row" role="tablist" aria-label="Effectbibliotheek">${tabs.map(key=>`<button role="tab" data-action="library" data-id="${key}" aria-selected="${tab===key}">${categoryLabel(key)}</button>`).join('')}</div>${active?'':intro}${tab!=='presets'?'<label class="animation-search"><span>Zoek in alle animaties</span><input type="search" id="animation-search" placeholder="Naam, kleur of beweging" autocomplete="off"></label>':''}<div id="animation-results">${results}</div></div>`;
  }
  ...
  function renderLayout() {
    const type = zone().type, choices=[['stacked','Onder elkaar','Een lichtlijn per rij.'],['vertical','Verticaal','Lijnen rechtop naast elkaar.']];
  ...
  function receiverOrderMarkup() {
    ...
    return `<p class="order-help">Laat een lijn knipperen om haar te herkennen. Sleep daarna aan de stippen naar de juiste plek.</p><div class="receiver-order" aria-label="Receivervolgorde">${list.map((r,i)=>{
      const blinking=identifying.get(r.id)?.scope==='all',disabled=r.type==='SPI'&&!r.outputs.some(o=>o.enabled);
      return `<div data-order-receiver="${esc(r.id)}"><button class="order-handle" aria-label="${esc(r.name)} verslepen" title="Sleep om te verplaatsen">⠿</button><span class="order-number">${i+1}</span><div class="order-name"><b>${esc(r.name)}</b><small>${r.type==='SPI'?`${r.outputs.filter(p=>p.enabled).reduce((a,p)=>a+p.pixels,0)} pixels`:'RGBW · één geheel'}</small></div><div class="order-controls"><button type="button" class="receiver-blink order-identify" data-action="visual-identify" data-receiver="${esc(r.id)}" aria-pressed="${blinking}" aria-label="${esc(r.name)} ${blinking?'stoppen met knipperen':'laten knipperen'}" ${disabled?'disabled':''}>${icon('sun')}<span>${blinking?'Stop knipperen':'Knipperen'}</span></button><div class="order-buttons"><button data-action="order-up" data-id="${esc(r.id)}" aria-label="${esc(r.name)} naar boven" ${i===0?'disabled':''}>↑</button><button data-action="order-down" data-id="${esc(r.id)}" aria-label="${esc(r.name)} naar beneden" ${i===list.length-1?'disabled':''}>↓</button></div></div></div>`;
    }).join('')}</div><p class="order-status" role="status" aria-live="polite"></p>`;
  }
  function productVisual(receiver,compact=false) {
    return `<div class="receiver-product ${compact?'compact list-product':''}"><canvas data-product-receiver="${esc(receiver.id)}" data-compact="${compact}" width="400" height="240" role="img" aria-label="${receiver.type==='RGBW'?'RGBW: voeding, controller en vier aansluitingen op één splitter':'SPI: schematische receiver met vier uitgangen'}"></canvas>${compact?'':`<p class="receiver-product-caption"><strong>${receiver.type==='RGBW'?'Eén receiver · één geheel':`Uitgang ${visualPorts.get(receiver.id)||1} geselecteerd`}</strong>${receiver.type==='RGBW'?'Alle aangesloten RGBW LED-lines volgen dezelfde kleur en animatie.':'Kies de uitgang die je wilt instellen.'}</p>`}</div>`;
  }
  function receiverCard(r) {
    const z=M.getZone(model,r.zoneId),rid=esc(r.id),port=visualPorts.get(r.id)||1;
    const wholeBlink=identifying.get(r.id)?.scope==='all';
    return `<details data-receiver-detail="${rid}" ${expandedReceivers.has(r.id)?'open':''}><summary>${productVisual(r,true)}<div class="receiver-heading-copy"><b>${esc(r.name)}${r.role==='main'?' · Hoofdreceiver':''}</b><small>${r.type} · ${esc(z?.name||'Nog geen zone')}</small></div><button type="button" class="receiver-blink" data-action="visual-identify" data-receiver="${rid}" aria-pressed="${wholeBlink}" aria-label="${esc(r.name)} · ${r.type==='SPI'?'alle actieve uitgangen samen':'hele receiver'} ${wholeBlink?'stoppen met knipperen':'laten knipperen'}">${icon('sun')}<span>${wholeBlink?'Stop':'Knipperen'}</span></button>${icon('chevron')}</summary><div class="receiver-details">${productVisual(r)}${r.type==='SPI'?`<div class="receiver-ports-heading"><h3>Uitgangen</h3><small>Alle ingeschakelde uitgangen lichten op in het voorbeeld.</small></div><div class="receiver-port-list" aria-label="Uitgang bekijken">${r.outputs.map(o=>`<div class="receiver-port-row ${o.port===port?'port-focused':''}" data-port-row="${o.port}"><button type="button" class="receiver-port-select" data-action="visual-port" data-receiver="${rid}" data-id="${o.port}" aria-pressed="${o.port===port}"><b>P${o.port}</b><span>Uitgang ${o.port}</span></button><button type="button" class="switch receiver-port-switch" role="switch" data-action="receiver-port-enabled" data-receiver="${rid}" data-port="${o.port}" aria-checked="${o.enabled}" aria-label="Uitgang ${o.port} gebruiken" ${nativeContext?'disabled title="Uitgangen wijzigen is nog niet aangesloten op de receiverdienst"':''}><span>${o.enabled?'Aan':'Uit'}</span><i aria-hidden="true"></i></button><button type="button" class="receiver-blink" data-action="port-identify" data-receiver="${rid}" data-port="${o.port}" aria-pressed="${identifying.get(r.id)?.scope===String(o.port)}" aria-label="${esc(r.name)} uitgang ${o.port} laten knipperen" ${o.enabled?'':'disabled'}>${icon('sun')}<span>${identifying.get(r.id)?.scope===String(o.port)?'Stop':'Knipperen'}</span></button></div>`).join('')}</div><button class="button full pixel-setup-shortcut" data-action="receiver-pixel-setup" data-id="${rid}">${icon('sliders')}Pixels / beginpunt instellen ${icon('chevron')}</button><p class="port-preview-note">Eerst het aantal pixels, daarna groen en rood vergelijken met je opstelling.</p>`:''}${receiverAssignmentActions(r)}${z&&route.screen==='receivers'?`<button class="text-button" data-action="zone" data-id="${esc(z.id)}">Bedien ${esc(z.name)} →</button>`:''}</div></details>`;
  }
  function receiverAssignmentActions(r) {
    return `<div class="receiver-management"><button class="button secondary" data-action="receiver-move" data-id="${esc(r.id)}">${icon('zones')}${r.zoneId?'Verplaatsen naar een zone':'Zone kiezen'}</button><button class="text-button" data-action="receiver-rename" data-id="${esc(r.id)}">Naam wijzigen</button>${r.zoneId?`<button class="text-button" data-action="receiver-unassign" data-id="${esc(r.id)}">Uit deze zone halen</button>`:''}<small>Verplaatsen verandert geen kleuren of poorten. Uit een zone halen is géén reset.</small></div>`;
  }
  function renderReceivers() {
    const all=standReceivers(),unassigned=all.filter(r=>!r.zoneId),shown=receiverFilter==='unassigned'?unassigned:all;
    const cards=shown.map(receiverCard).join('');
  ...
  function renderScenes() {
    ...
    return `<div class="page scenes-page"><header class="page-heading"><div><div class="eyebrow">${esc(standLabel())}</div><h1>Scènes</h1><p>Je ingestelde verlichting bewaren en later weer gebruiken.</p></div></header>${sceneWorkflow()}<button class="button red" data-action="scene-new" ${ready?'':'disabled'}>＋ Nieuwe scène</button>${savedScenes.error?`<p class="card" role="alert">${esc(savedScenes.error.message)}</p>`:''}<div class="section-heading scene-list-heading"><h2>Jouw scènes</h2><small>${list.length} ${list.length===1?'scène':'scènes'}</small></div><div class="scene-list">${list.map(scene=>{
      ...
      return `<button class="scene-card" data-action="scene-open" data-id="${esc(scene.id)}">${scenePreview(scene)}<span class="scene-card-copy"><b>${esc(scene.name)}</b><small class="scene-card-count">${zoneCount(scene.zones.length)} · ${count} receivers</small><small class="scene-card-names">${esc(names)}${extra>0?' …':''}</small><span class="scene-open-label">Bekijk ${zoneCount(scene.zones.length)} ${icon('chevron')}</span></span></button>`;
    }).join('')||`<section class="card empty">${icon('scenes')}<h2>Bewaar je eerste sfeer</h2><p>${ready?'Zijn je kleuren en animaties ingesteld? Kies Nieuwe scène en selecteer de zones die je wilt bewaren.':'Voeg eerst verlichting aan een zone toe via Stand. Daarna kun je een scène bewaren.'}</p></section>`}</div><p class="scene-reading-note">Bekijken en opslaan veranderen niets. Alleen ‘Scène activeren’ past de opgeslagen verlichting toe.</p></div>`;
  }
  ...
  function syncSceneDraft() {
    ...
    const summary=main.querySelector('[data-scene-selection-summary]');if(summary)summary.textContent=`${zoneCount(chosen.length)} gekozen · ${total} receivers`;
  ...
  function render({top=false,preserveScroll=false}={}) {
    ...
    if(!model.stands.length&&!['stand','scenes','settings','pin-login','receivers','receiver-add'].includes(route.screen))route.screen='stand';
    ...
    if(route.screen==='animations'&&zone()&&receivers().length&&!activeEffect()){
      route={...route,screen:'effects',family:null,library:'all'};top=true;
    }
    const focused=document.activeElement,focusKey=focused?.dataset?.id;
    previews.clear();
    const views = {stand:renderStand,controls:renderControls,colour:renderColour,animations:renderAnimations,effects:renderEffects,layout:renderLayout,receivers:renderReceivers,settings:renderSettings,'pin-login':renderPinLogin,scenes:renderScenes,'scene-draft':renderSceneDraft,'scene-detail':renderSceneDetail,'receiver-add':renderReceiverAdd};
    ...
    if(route.screen==='settings')main.querySelector('[data-action="help"]')?.insertAdjacentHTML('afterend',
    ...
    if(nativeContext&&route.screen==='settings'){
      const info=main.querySelector('#connection-info'),paragraphs=info?.querySelectorAll(':scope > p');
      if(paragraphs?.[0])paragraphs[0].textContent=runtime?.native===true?'Deze V30-bouw gebruikt de iPhone-verbindingsdienst. Kies voorlopig zelf het receiver-wifinetwerk in Instellingen → Wifi en keer daarna terug. Alleen zoeken voegt niets toe.':'De iPhone-verbindingsdienst is niet beschikbaar. Er worden geen receiveropdrachten verstuurd.';
      if(paragraphs?.[1])paragraphs[1].textContent='Je stand en onafgeronde toevoeging worden lokaal bewaard. Een nieuwe hoofdreceiver wordt pas zichtbaar na beveiligde bevestiging. Een back-up via je PIN is een aparte functie en is nog niet aangesloten; er wordt niets uit een bestaande installatie overgenomen of gewist.';
      const readiness=info?.querySelectorAll('.readiness-list li')[1];
      if(readiness){readiness.querySelector('b').textContent=runtime?.native===true?'Zoeken beschikbaar · receiverbeheer ingebouwd':'Verbindingsdienst niet beschikbaar';readiness.querySelector('span').textContent=runtime?.native===true?'MAIN-koppeling, lokaal hervatten, MAIN-updates en het verwijderen van hoofd- en extra receivers zijn aangesloten op de native diensten. Het hele netwerk wissen vraagt je PIN en verwijdert de hoofdreceiver als laatste. Fysieke acceptatie moet nog gebeuren. Live lichtbediening, extra ESP-NOW-receivers toevoegen en PIN-installatieherstel zijn nog niet vrijgegeven. Een softwaretest is geen fysieke verbindingstest.':'Deze app laadt geen voorbeeldreceivers als de verbindingsdienst ontbreekt. Er worden geen receiveropdrachten uitgevoerd; de bestaande installatie blijft intact.';}
      ...
      const readiness=info?.querySelectorAll('.readiness-list li')[1];
      if(readiness){readiness.querySelector('b').textContent='Verbindingsdienst niet beschikbaar';readiness.querySelector('span').textContent='Er worden geen receiveropdrachten verstuurd en geen koppelingen nagebootst. Bestaande installaties en PINs op je receivers blijven intact.';}
    }
    main.querySelectorAll('[data-receiver-detail]').forEach(card=>{
      ...
      if(r.role==='main')card.querySelector('.receiver-details').insertAdjacentHTML('beforeend',`<button class="button secondary full" data-action="receiver-update" data-id="${esc(r.id)}">Softwareversie en updates</button>`);
      card.querySelector('.receiver-details').insertAdjacentHTML('beforeend',`<button class="button secondary full" data-action="receiver-remove" data-id="${esc(r.id)}">${r.role==='main'?'Hoofdreceiver en netwerk verwijderen':'Receiver volledig verwijderen'}</button>`);
    });
    ...
    if(route.screen==='layout')main.querySelector('.receiver-order')?.replaceWith(document.createRange().createContextualFragment(receiverOrderMarkup()));
    ...
    const measureContext=()=>document.documentElement.style.setProperty('--sticky-height',context&&getComputedStyle(context).position==='sticky'?`${Math.ceil(context.getBoundingClientRect().height)}px`:'0px');
    measureContext();
    if(context){contextObserver=new ResizeObserver(measureContext);contextObserver.observe(context);}
    ...
    if(route.screen==='stand'&&stand()){
      ...
      main.querySelector('.zone-grid')?.closest('section').insertAdjacentHTML('afterend',`<button class="button secondary full" data-action="receiver-add">${onboarding.summary()?'Verder met receiver toevoegen':'＋ Receiver toevoegen'}</button>`);
    }
  ...
  function apply(patch) {
    model=standControlOpen?M.applyStandState(model,stand().id,patch):M.applyState(model,route.zoneId,selection(),patch);
  ...
  function staticColour(rgb,w=0) { apply({...C.state(rgb,w),engine:'STATIC',variant:0,v30Effect:null,category:null,animation:'Static Color',previewFamily:null,legacySpi:false,bounce:false,mirror:false,on:true,power:true});syncColour(); }
  ...
  function writePicker(root,rgb,w,changed='both') {
    if(root?.dataset.colourPicker==='background'){
      ...
      apply({background:changed==='white'?rawBackground:C.hex(rgb),backgroundWhite:changed==='rgb'?rawWhite:w,
        ...(changed==='white'?{}:{backgroundRgbEnabled:true}),...(changed==='rgb'?{}:{backgroundWhiteEnabled:true})});
  ...
  function syncColour() {
    document.querySelectorAll('[data-colour-picker]').forEach(root=>{
      ...
      ['r','g','b','w'].forEach((key,i)=>{const input=root.querySelector(`input[data-channel="${key}"]`),out=root.querySelector(`[data-channel-value="${key}"]`);if(input)input.value=values[i];if(out)out.textContent=values[i];});
    });
  }
  function paintWheel() {
  ...
  function showHelp() {
    document.getElementById('help-content').innerHTML=`<header><h2>Kies de plek.<br>Bedien het licht.</h2><button class="icon-button" data-action="close-help" aria-label="Uitleg sluiten">${icon('close')}</button></header><section>${icon('stand')}<div><h3>Stand → je hele installatie</h3><p>Alles wat bij jouw beursstand hoort, bij elkaar.</p></div></section><section>${icon('zones')}<div><h3>Zone → een plek in je stand</h3><p>Bijvoorbeeld Demohoek, Balie of Plafond. Open een zone om meteen het licht te bedienen.</p></div></section><section>${icon('receiver')}<div><h3>Samen of één receiver</h3><p>Kies Samen voor de hele zone. Bij losse lijnen kun je één receiver kiezen; de actieve lijn wordt gemarkeerd. Doorlopende SPI bedien je altijd samen.</p></div></section><aside class="guide"><p>RGBW en Pixel LED (SPI) krijgen ieder hun eigen zone. Zo zie je alleen de passende bediening.</p></aside><button class="button" data-action="close-help">Begrepen</button>`;
  ...
  function showStandControls(){
    ...
    showEffectDialog('Alles bedienen',`<p class="stand-control-scope"><b>${esc(standLabel())}</b> · ${stand().zones.length} zones · ${standReceivers().length} receivers</p><p>Een vaste kleur voor je hele stand, zowel RGBW als SPI. Aan/uit bewaart je huidige kleuren en animaties.</p>${powerControl()}<p id="stand-control-mixed" class="mixed-note" ${mixedSelection()?'':'hidden'}>Je verlichting heeft verschillende instellingen. Een kleur kiezen maakt alles dezelfde vaste kleur.</p>${colourPickerMarkup()}<button class="button full" data-action="effect-dialog-close">Klaar · terug naar mijn stand</button>`);
  ...
  function showPaletteEditor(index) {
    ...
    showEffectDialog(`Kleur ${index+1} aanpassen`,`${dialogAnimationPreview()}${colourPickerMarkup(index)}<button class="button full" data-action="effect-dialog-close">Klaar</button>`);
    paintWheel();syncColour();
  }
  ...
  function showSavePreset() {
    ...
    if(mixedSelection())return toast('Kies één receiver, of stel Samen eerst gelijk in. Zo is duidelijk wat je bewaart.');
    showEffectDialog('Bewaar jouw animatie',`<p>${esc(effect.name)} · alle gekozen kleuren en instellingen worden bewaard. Je past de preset later toe op je dan geselecteerde receiver(s).</p><label class="dialog-field">Naam van je preset<input id="preset-name" type="text" maxlength="64" placeholder="Bijvoorbeeld: zacht welkom" autocomplete="off"></label><p class="dialog-error" role="alert" hidden></p><button class="button full" data-action="preset-confirm" disabled>Preset opslaan</button>`);
  ...
  function showReceiverAssignment(receiverId,targetZoneId=null) {
    const r=model.receivers.find(item=>item.id===receiverId&&item.lifecycle==='added'&&item.standId===stand().id);if(!r)return;
    receiverAssignment={receiverId,zoneId:targetZoneId||r.zoneId};
    showEffectDialog(`Waar hoort ${r.name}?`,`<p>${esc(r.type)} · nu ${r.zoneId?`in ${esc(M.getZone(model,r.zoneId).name)}`:'nog niet aan een zone toegewezen'}. Kies zijn nieuwe plek; de lichtinstellingen blijven bewaard.</p><div class="assignment-choices" aria-label="Zone kiezen">${stand().zones.map(z=>{
      const compatible=!z.type||z.type===r.type,chosen=z.id===receiverAssignment.zoneId;
      return `<button class="assignment-choice" data-action="assignment-zone" data-id="${esc(z.id)}" aria-pressed="${chosen}" ${compatible?'':'disabled'}>${icon('zones')}<span><b>${esc(z.name)}</b><small>${z.id===r.zoneId?'Huidige zone':compatible?`${zoneTypeLabel(z)}${z.type?` · ${z.receiverIds.length} receivers`:''}`:`Alleen ${z.type} · past niet bij deze receiver`}</small></span><i aria-hidden="true">${chosen?'✓':''}</i></button>`;
    }).join('')}</div><button class="button secondary full" data-action="assignment-new-zone">＋ Nieuwe zone maken</button><p class="dialog-error" role="alert" hidden></p><button class="button full" data-action="assignment-confirm" ${!receiverAssignment.zoneId||receiverAssignment.zoneId===r.zoneId?'disabled':''}>${r.zoneId?'Verplaatsen':'Toewijzen'}</button>`);
  }
  ...
  async function manageSetupZones(request) {
    ...
    const keys=request.kind==='delete'?['kind','zoneId','expectedZoneSignature']:request.kind==='assign'?['kind','receiverId','zoneId',...(request.zone===undefined?[]:['zone'])]:[];
    ...
    if(request.kind==='delete'){
      ...
      next=M.deleteZone(model,existing.id);operation={kind:'delete',zoneId:existing.id};
    }else{
    ...
    try{
      if(nativeContext){
        ...
        confirmedView=await runtime.services.editZones({standId:s.id,operation,...(request.kind==='delete'?{expectedZoneSignature:request.expectedZoneSignature}:{})});
        next=keepLocalPreviewStates(confirmedView.model);
      }
      model=next;selections.clear();zoneDeletion=null;receiverAssignment=null;nameDialog=null;
      if(request.kind==='delete'){
      ...
      if(view?.model){
        model=keepLocalPreviewStates(view.model);selections.clear();zoneDeletion=null;receiverAssignment=null;nameDialog=null;
  ...
  async function moveReceiver(id,toIndex) {
    ...
    if(!saved)return;model=saved;render();
  ...
  document.addEventListener('click',async event=>{
    ...
    try {
      ...
      if(action==='receiver-pixel-setup'){const receiver=model.receivers.find(r=>r.id===id&&r.type==='SPI'&&r.lifecycle==='added');if(receiver){pixelSetupReceiverId=id;return pixelSetup.open(receiver,{initialPort:visualPorts.get(id)||1});}return;}
      ...
      if(action==='assignment-zone'){
        ...
        const confirm=document.querySelector('[data-action="assignment-confirm"]');confirm.disabled=id===r.zoneId;confirm.textContent=`${r.zoneId?'Verplaatsen':'Toewijzen'} naar ${M.getZone(model,id).name}`;return;
      }
      ...
      if(action==='assignment-confirm'){
        const {receiverId,zoneId}=receiverAssignment;
        return await updateManagement(M.assignReceiverToZone(model,receiverId,zoneId),`Verplaatst naar ${M.getZone(model,zoneId).name}.`,receiverId,{kind:'assign',receiverId,zoneId});
      }
      ...
      if(action==='select'){
        ...
        selections.set(route.zoneId,id==='all'?{kind:'all'}:{kind:'receiver',receiverId:id});render();
      ...
      if(action==='swatch'){
        ...
        else {apply(Colours.restore(entry));syncColour();const slider=document.querySelector('[data-setting="bri"]'),out=document.querySelector('[data-value-for="bri"]');if(slider)slider.value=entry.color.bri;if(out)out.textContent=entry.color.bri+'%';}return;
      }
      ...
      if(action==='scene-apply'){const scene=savedScenes.scenes.find(s=>s.id===id&&s.standId===stand().id);if(!scene)return;model=Scenes.apply(model,scene);render();toast('Scène geactiveerd.');return;}
      ...
      if(action==='preset-confirm'){
        try {
          ...
          savedPresets=result;closeEffectDialog();navigate('effects',{library:'presets',family:null});toast('Preset opgeslagen in Eigen presets.');
      ...
      if(action==='preset-delete'){
        const preset=savedPresets.presets.find(p=>p.id===id);if(!preset)return;
        showEffectDialog('Preset verwijderen?',`<p>“${esc(preset.name)}” verdwijnt uit Eigen presets. Het huidige licht verandert niet.</p><button class="button full" data-action="preset-delete-confirm" data-id="${esc(id)}">Preset verwijderen</button><button class="button secondary full" data-action="effect-dialog-close">Behouden</button>`);return;
      }
      ...
      if(action==='setting-reset'){
        ...
        const value=effect.state[id]??(id==='bri'?100:id==='bgBrightness'?10:['bounce','mirror'].includes(id)?false:undefined);if(value===undefined)return;
      ...
      if(action==='direction'){apply({direction:button.dataset.value});document.querySelectorAll('[data-action="direction"]').forEach(el=>el.setAttribute('aria-pressed',el===button));return;}
      if(action==='effect-boolean'){if(!['bounce','mirror'].includes(id)||!activeEffect()?.controls.includes(id))return;const value=!selectedState()[id];apply({[id]:value});button.setAttribute('aria-pressed',value);button.querySelector('b').textContent=value?'Aan':'Uit';return;}
      if(action==='set-layout'){
        ...
        if(!next)return;model=next;if(continuousZone())selections.set(route.zoneId,{kind:'all'});return render();
      }
  ...
  document.addEventListener('input',event=>{
    ...
    try {
      ...
      if(input.dataset.setting){const key=input.dataset.setting;apply({[key]:Number(input.value)});document.querySelector(`[data-value-for="${key}"]`).textContent=input.value+(input.dataset.unit||'');}
