...
(function(root,factory){'use strict';root.LightningOnboardingUI=factory();}(window,function(){
  ...
  const labels={stand:'Je stand',zones:'Zones maken',receiver:'Receiver zoeken',outputs:'Kies je uitgangen',pixels:'Stel je lengte in',connection:'Kies het beginpunt',pin:'Kies je installatie-PIN',security:'Verbinding bevestigen',zone:'Kies de zone',review:'Klaar om toe te voegen',done:'Je receiver is klaar'};
  ...
  function connectionFailure(failure){
    ...
    if(['MAIN_IDENTITY_UNCONFIRMED','IDENTITY_MISMATCH','DISCOVERY_INVALID'].includes(code))return {message:'De identiteit van deze receiver is nog niet bevestigd. Controleer of je het ALUVISION-wifinetwerk van de juiste receiver hebt gekozen en tik op Verbinding controleren. Er is niets toegevoegd of gereset.'};
  ...
  function create({draftApi=window.LightningOnboardingDraft,visual=window.LightningReceiverVisual,pixelSetup=window.LightningPixelSetup,services={},getModel,onComplete=()=>{},onManage=null,onExit=()=>{}}={}){
    ...
    let zoneExtraNames=[],zoneRemoval=null,receiverMove=null,managementBusy=false;
    const visualEntrances=new Map(),presentedStages=new Set();
    ...
    const canIdentify=receiver=>typeof services.identify==='function'&&receiver.canConfigure!==false;
    ...
    function verifyAgain(value){
      resumeSelection=clone(value);resumeSelection.security={status:'not-started',phase:'idle'};
      if(['security','zone','review'].includes(resumeSelection.stage))resumeSelection.stage='pin';
    ...
    function start({activeZoneId}={}){
      if(!draft||draft.stage==='done'){
        ...
        zoneExtraNames=[];zoneRemoval=null;receiverMove=null;managementBusy=false;
        visualEntrances.clear();presentedStages.clear();
      }else if(draft.cancelled)change({type:'RETRY'},{render:false});
    }
    function mount(element,options){start(options);origin=options?.origin==='receivers'&&draft.mainReceiverId?'receivers':'stand';container=element;container.addEventListener('click',click);container.addEventListener('input',input);container.addEventListener('keydown',keydown);paintPage(false);}
    function suspend(){
    ...
    function heading(){
      ...
      const title=draft.stage==='stand'?'Hoe heet je stand?':draft.stage==='zones'?(zoneRemoval?'Zones beheren':receiverMove?'Receiver verplaatsen':zoneNames().length>1?'Maak je zones':zoneNameOpen&&draft.zones.length?'Nieuwe zone':draft.zones.length?'Je zones staan klaar':'Maak je eerste zone'):draft.stage==='receiver'?(busy?'Receiver controleren':searchState==='searching'?'Receiver zoeken…':results.length===1?'Receiver gevonden':results.length>1?'Kies je receiver':'Receiver zoeken'):labels[draft.stage]||'Receiver toevoegen';
    ...
    function zoneRemovalPanel(){
      ...
      return `<section class="card onboarding-zone-confirm" data-setup-delete-zone="${escape(zone.id)}"><h2>${escape(zone.name)} verwijderen?</h2><p>${count?`${count} ${count===1?'receiver blijft':'receivers blijven'} gekoppeld en komt bij <b>Niet in een zone</b>. Je kunt ze meteen opnieuw indelen.`:'Je verwijdert alleen deze zone.'}</p><small>Je receivers en PIN worden niet gereset.</small><div class="onboarding-actions">${button('zone-remove-cancel','Annuleren','class="button secondary"')}${button('zone-remove-confirm','Zone verwijderen')}</div></section>`;
    }
    ...
    function receiverAction(receiver){
      return button('receiver',busy?'Controleren…':'Deze receiver instellen',`data-id="${escape(receiver.id)}" ${busy||!(receiver.canConfigure||receiver.canVerifyIdentity)?'disabled':''}`);
    }
    ...
    function lightExample(type){
      ...
      return `<figure class="onboarding-light-example" data-setup-visual="${type==='SPI'?'pixels':'whole-line'}" aria-label="${type==='SPI'?'Voorbeeld: licht beweegt per pixel. Geen live weergave.':'Voorbeeld: alle RGBW-lijnen veranderen samen van kleur. Geen live weergave.'}"><div aria-hidden="true">${type==='SPI'?Array.from({length:20},(_,i)=>`<i style="--pixel:${i}"></i>`).join(''):'<i></i><i></i>'}</div><figcaption>Voorbeeld · ${type==='SPI'?'licht per pixel':'alle lijnen samen'}</figcaption></figure>`;
    }
    ...
    function pinForm(){
      return `<p class="onboarding-task-hint">Eén PIN voor je hele installatie.</p><section class="card onboarding-pin-card"><label class="onboarding-field">PIN · 8–12 cijfers<input id="onboarding-pin" data-onboarding-pin="first" type="password" inputmode="numeric" autocomplete="new-password" minlength="8" maxlength="12" aria-describedby="onboarding-pin-help" spellcheck="false"></label><label class="onboarding-field">Herhaal dezelfde PIN<input id="onboarding-pin-repeat" data-onboarding-pin="repeat" type="password" inputmode="numeric" autocomplete="new-password" minlength="8" maxlength="12" aria-describedby="onboarding-pin-error onboarding-pin-match" spellcheck="false"></label><p id="onboarding-pin-error" class="onboarding-error" role="alert" hidden></p><p id="onboarding-pin-match" class="onboarding-pin-match" role="status" hidden>✓ Beide PINs komen overeen</p><p id="onboarding-pin-help" class="onboarding-hint">Bewaar je PIN: ook je wifi-wachtwoord en toegang op een ander toestel.</p></section><details class="onboarding-recovery"><summary>Als je de PIN vergeet</summary><p>Je hebt dan een ondersteunde herstel- of fabrieksresetprocedure op de receiver nodig. Er wordt hier geen verborgen standaard-PIN ingesteld.</p></details>${footerPin()}`;
    }
    function reviewCard(){
      return `<section class="card onboarding-review onboarding-review-product">${product(draft.receiver,{compact:true,port:null})}<dl><div><dt>Receiver</dt><dd>${escape(draft.receiver.name||draft.receiver.type)}</dd></div><div><dt>Zone</dt><dd>${escape(draft.zones.find(zone=>zone.id===draft.zoneId)?.name)}</dd></div><div><dt>Verbinding</dt><dd>${draft.role==='main'?'Hoofdreceiver · beveiligd':'Via hoofdreceiver'}</dd></div></dl>${draft.receiver.type==='SPI'?`<ul>${active().map(output=>`<li><b>P${output.port}</b><span>${output.pixels} pixels<small>${pixelSetup.endpointLabel(output)}</small></span></li>`).join('')}</ul>`:lightExample('RGBW')}</section><div class="onboarding-actions onboarding-footer">${button('back','← Zone kiezen',busy||finalizationStarted?'disabled':'')}${button('finish',busy?'Toevoegen…':'Receiver toevoegen',busy?'disabled':'')}</div>`;
    }
    ...
    function securityPanel(){
      if(manualRejoinSSID)return `<section class="card onboarding-manual-wifi" aria-label="Met beveiligde wifi verbinden"><h2>Verbind opnieuw met wifi</h2><ol class="onboarding-wifi-tiles" data-setup-visual="wifi-rejoin"><li>${setupIcon('wifi')}<span><b>Instellingen → Wifi</b><small>Kies ${escape(manualRejoinSSID)}</small></span></li><li>${setupIcon('phone')}<span><b>Vul je eigen PIN in</b><small>Dit is nu je wifi-wachtwoord.</small></span></li><li>${setupIcon('return')}<span><b>Terug naar de app</b><small>Controleer de verbinding hieronder.</small></span></li></ol></section>${button('security-retry',busy?'Verbinding controleren…':'Ik ben verbonden · controleren',busy?'disabled':'')}<small class="onboarding-hint">Dezelfde receiver en PIN. Je keuzes blijven bewaard.</small>`;
    ...
    function body(){
      switch(draft.stage){
        ...
        case 'zones':{
          ...
          return `${draft.zones.length?'':`<p class="onboarding-task-hint">Zone = plek in je stand, zoals de balie.</p>`}${setupZones({editable:!editing})}${draft.zones.length?`<p class="onboarding-zone-count" role="status">${draft.zones.length} ${draft.zones.length===1?'zone':'zones'} · Kies waar je begint.</p>`:''}${editing?`${zoneNameFields()}<div class="onboarding-actions onboarding-footer">${button(draft.zones.length?'zone-cancel':'back',draft.zones.length?'Annuleren':'← Terug','class="button secondary"')}${button('zone-create',zoneNames().length>1?`${zoneNames().length} zones maken`:'Zone maken','disabled')}</div>`:`<p class="onboarding-later-hint">Zones toevoegen of receivers verplaatsen kan ook later.</p>${button('zone-new','＋ Zones toevoegen','class="button secondary"')}${receiverPlacement()}${footer('Verder · receivers toevoegen')}`}`;
        }
        ...
        case 'pin':return pinForm();
        case 'security':return securityPanel();
        case 'zone':return `<div class="onboarding-zone-list">${zoneChoices()}</div>${zoneNameOpen?`<section class="card">${nameField('onboarding-zone-name','Naam van de nieuwe zone','Bijvoorbeeld: Lichttunnel',zoneNameInput)}${button('zone-create','Zone maken','disabled')}</section>`:button('zone-new','＋ Nieuwe zone','class="button secondary"')}${button('next','Verder naar overzicht',draft.zoneId?'':'disabled')}<small class="onboarding-hint">RGBW en SPI apart. Verplaatsen kan later.</small>`;
    ...
    function updateGuidance(){
      ...
      if(busy||draftSaving||pendingChoices||saveFailed||error||zoneRemoval||receiverMove)return;
      let action=null;
      switch(draft.stage){
    ...
    function paint(time){
      ...
      if(draft.stage==='outputs'){
        ...
        pixelSetup.paintOutputs(container,draft.outputs,{time,reducedMotion,visual,entranceProgress});
      ...
      for(const [id,until] of identifying)if(until<=time){
        ...
        if(control){control.textContent='Knipperen';control.setAttribute('aria-pressed','false');const label=control.closest('.onboarding-recognition')?.querySelector('span');if(label)label.textContent='Herken jouw verlichting';}
      }
    ...
    function keydown(event){if(event.key==='Enter'&&event.target.matches('input:not([data-onboarding-pin])')){const button=container.querySelector('[data-onboarding-action="stand-save"], [data-onboarding-action="zone-create"]');if(button&&!button.disabled){event.preventDefault();button.click();}}}
    async function search(){
      ...
      try{
        const result=await bounded(services.search({standId:draft.stand.id,mainReceiverId:draft.mainReceiverId,signal:searchAbort.signal}));
        ...
        searchState='ready';
      }catch(failure){if(token!==operation)return;searchState='failed';results=[];error=connectionFailure(failure).message;}
    ...
    async function stopIdentify(id,knownReceiver=null){
      ...
      if(receiver&&typeof services.identify==='function')try{const answer=await bounded(services.identify({receiver:clone(receiver),enabled:false,ttlMs:10000}));if(answer?.confirmed!==true)throw Error('UNCONFIRMED');}catch(_){notice='Stoppen is nog niet bevestigd. Controleer de receiver; verder instellen blijft mogelijk.';}
    }
    async function identify(id){
      const receiver=results.find(receiver=>receiver.id===id);if(!receiver||receiver.canConfigure===false||identifyPending.has(id))return;
      if(typeof services.identify!=='function'){notice='Knipperen is niet beschikbaar zonder receiververbinding.';paintPage(false);return;}
      ...
      try{
        if(stopping)await stopIdentify(id);
          ...
          const answer=await bounded(services.identify({receiver:clone(receiver),enabled:true,ttlMs:10000}));if(answer?.confirmed!==true)throw Error('UNCONFIRMED');
          if(identifyOperation!==operation||!container||draft.stage!=='receiver')await stopIdentify(id,receiver);
          else identifying.set(id,performance.now()/1000+10);
        }
    ...
    async function secure(pin,repeat,{reconcile=false}={}){
      if(busy)return;
      ...
      repeat='';busy=true;error='';const token=++operation;paintPage();
      ...
      try{
        ...
        if(method==='reconcileSecurity'&&securityReceiptRef)response={receiptRef:securityReceiptRef};
          ...
          attempted=true;const request=services[method]({configuration:config(),...(method==='secure'&&draft.role==='main'?{pin}:{}),onProgress:phase=>{
            if(token===operation&&busy&&draft.stage==='security'&&phaseLabels[phase])change({type:'SECURITY_PROGRESS',phase},{top:false});
          }});
          pin='';repeat='';response=await bounded(request,50000);
        }
        pin='';repeat='';
        ...
        if(response?.status==='manual-rejoin'){
          ...
          manualRejoinSSID=response.ssid;return;
        }
        ...
        if(!attempted&&!securityUncertain&&draft.role==='main')draft=draftApi.snapshot({...clone(draft),stage:'pin',security:{status:'not-started',phase:'idle'}});
        error='De beveiligde verbinding is nog niet bevestigd. Je keuzes blijven bewaard. Controleer opnieuw om dezelfde koppeling te hervatten.';
      }
      finally{pin='';repeat='';busy=false;keepDraft();paintPage();}
    }
    async function finish(){
      if(busy)return;busy=true;finalizationStarted=true;error='';const token=++operation;paintPage(false);
      try{
        await persist();if(token!==operation)throw Error('SUSPENDED');
        let response;
      ...
      }catch(_){error='Toevoegen is nog niet bevestigd. Je receiver staat nog niet in de lijst. Je keuzes blijven bewaard; probeer opnieuw.';}
    ...
    async function manageSetup(request){
      ...
      try{
        ...
        draft=refreshed.draft;zoneRemoval=null;receiverMove=null;notice='';
    ...
    async function click(event){
      ...
      if(action==='zone-create'){
        ...
        return saveChoices(zoneNames().map(name=>({type:'ADD_ZONE',id:'zone-'+crypto.randomUUID(),name})),()=>{
      ...
      if(action==='zone-remove-confirm'){
        ...
        if(zone.isNew)return saveChoices([{type:'REMOVE_ZONE',zoneId:zone.id}],()=>{zoneRemoval=null;resetZoneNames();},{top:false});
      ...
      if(action==='receiver'){
        ...
        if(receiver.canConfigure===false&&receiver.canVerifyIdentity){
          ...
          try{
            const verified=await bounded(services.select({standId:draft.stand.id,transactionId:draft.transactionId,receiver:clone(expected)}),20000);
            ...
            receiver={id:verified.id,rid:verified.rid,type:verified.type,name:expected.name,deviceFingerprint:verified.deviceFingerprint,canConfigure:true};
            results=results.map(item=>item.id===receiver.id?receiver:item);
          }catch(failure){if(token===operation){const problem=connectionFailure(failure);error=problem.message;if(problem.expired){results=[];searchState='idle';}}return;}
        ...
        if(change({type:'SELECT_RECEIVER',receiver:candidate},{render:false})){for(const id of identifying.keys())stopIdentify(id);change({type:'NEXT'});if(draft.stage==='security')return secure();}return;
      }
      if(action==='count')return change({type:'SET_OUTPUT_COUNT',count:Number(target.dataset.count)});
      if(action==='output'){const output=draft.outputs.find(o=>o.port===Number(target.dataset.port));if(output)return change({type:'SET_OUTPUT_ENABLED',port:output.port,enabled:!output.enabled},{top:false});return;}
      ...
      if(action==='zone')return change({type:'SELECT_ZONE',zoneId:target.dataset.id});
      if(action==='back'){
      ...
      if(action==='security-retry')return secure(undefined,undefined,{reconcile:true});
    ...
    return Object.freeze({mount,suspend,paint,restore,summary});
  }
