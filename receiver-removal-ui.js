...
(function(root){
  ...
  function create({services={},getModel,onRemoved=()=>{}}={}){
    ...
    function consentReady(){
      const pinNeeded=installation()&&(result?.status==='ready'||result?.status==='pin-required');
    ...
    function validate(next,action,previous){
      if(!next||next.standId!==receiver.standId||next.receiverId!==receiver.id||next.rid!==receiver.rid||next.type!==receiver.type||next.scope!==scope()||!statuses.has(next.status)||next.requiresPin!==installation())throw Error('UNCONFIRMED');
    ...
    async function perform(action,automatic=false){
      ...
      if(installation()&&(action==='start'||result?.status==='pin-required'))request.pin=dialog.querySelector('[data-removal-pin]').value;
